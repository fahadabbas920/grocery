...
function ImageUploadZone({
  ...
  return (
    <div className="relative">
      <label className="group flex h-44 w-full cursor-pointer flex-col items-center justify-center overflow-hidden rounded-xl border-2 border-dashed border-(--color-border) bg-(--color-muted)/40 transition hover:border-(--color-primary) hover:bg-(--color-muted)/60">
        {preview ? (
...
export function ProductFormSheet({ categories, open, onOpenChange, product }: ProductFormSheetProps) {
  ...
  function clearFile() { setFile(null); setPreview(product?.imageUrl ?? null); }
  ...
  async function onSubmit() {
    ...
    try {
      ...
      if (isEdit) {
        const { error: e } = await supabase.from("products").update(input).eq("id", product.id);
        if (e) throw e;
        const { error: ie } = await supabase
          .from("inventory")
          .update({ quantity: Number(quantity), is_out_of_stock: outOfStock })
          .eq("product_id", product.id);
        if (ie) throw ie;
        toast.success("Product updated");
      } else {
        const { data: row, error: e } = await supabase.from("products").insert(input).select("id").single();
        if (e) throw e;
        ...
        <div className="flex flex-1 flex-col gap-6 overflow-y-auto px-6 py-5">
          ...
          <div className="space-y-4">
            ...
            <div className="grid gap-1.5">
              ...
              <Input placeholder="e.g. Full Cream Milk 1L" value={name} onChange={(e) => setName(e.target.value)} />
            </div>
            <div className="grid gap-1.5">
          ...
          <div className="space-y-4">
            ...
            <div className="grid gap-1.5">
              ...
              <Input type="number" min="0" value={quantity} onChange={(e) => setQuantity(e.target.value)} className="w-32" />
            </div>
            <div className="flex items-center justify-between rounded-xl border border-(--color-border) bg-muted/30 px-4 py-3">
              <div>
                ...
                <p className="text-xs text-(--color-muted-foreground)">Hides the add-to-cart button in the shop</p>
              </div>
